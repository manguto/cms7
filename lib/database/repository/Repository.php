...
class Repository extends Model 
{
    ...
    private function CtrlParametersInitialize()
    {
        ...
        foreach ($cps as $cp) {
            $key = RepositoryReferences::ctrl_parameter_ini . $cp;
            $this->values[$key] = '';
        }
    }
        ...
        { // definicao dos valores a serem ordenados
            ...
            { // ctrl parameters
                foreach (self::CtrlParameters as $ctrl_parameter_name) {
                    $key = RepositoryReferences::ctrl_parameter_ini . $ctrl_parameter_name;
                    $values_ord[$key] = null;
                }
            }
            // deb($values_ord);
    ...
    {
        $repository = Repository::getRepository($repositoryname, $condition);
        // deb($repository);
    ...
    {
        $FULL_REPOSITORY_ARRAY = Repository::getRepositoryARRAY($repositoryname);
        ...
        { // -------------------------------------------------------------------- ORDER BY ACTION
            ...
            if ($orderby !== false) {
                $FULL_REPOSITORY_ARRAY = Repository::SortBy($FULL_REPOSITORY_ARRAY, $orderby);
            }
        } // ------------------------------------------------------------------------------------
    ...
    private static function getRepositoryCSV(string $repositoryname): string
    {
        ...
        Repository::saveRepositoryCSV_START($repositoryname, $repositoryFilename);
        ...
        if (! $arquivoExiste) {
            ...
            foreach (array_keys($data) as $key) {
                ...
                { // evita o salvamento dos parametros de controle
                    $ctrl_left = RepositoryReferences::ctrl_parameter_ini;
    ...
    {
        $this_loaded = RepositoryReferences::Load($this);
    ...
    static function inicializar()
    {
        try {
            ...
            if (! Repository::repositoryFileExist($repositoryname)) {
                ...
                Repository::getRepositoryLength($repositoryname);
